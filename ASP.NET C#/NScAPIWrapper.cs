    ...
    class NScAPIWrapper
    {
        ...
        */
        ...
        private void buildRequestAndExecute(NameValueCollection parameters, string action) 
        {
            ...
            HttpWebRequest request = (HttpWebRequest) WebRequest.Create("https://api.nitrosell.com/" + this._webstoreUrl + "/v1/"+action+".json");
            request.Method = "POST";
        ...
        public void getCustomerDetails(string email) 
        {
            ...
            this._hmacmd5.ComputeHash(messageBytes);
            HttpWebRequest request = (HttpWebRequest) WebRequest.Create("https://api.nitrosell.com/" + this._webstoreUrl + "/v1/customer.json?email=" + email + "&hash=" + ByteToString(this._hmacmd5.Hash) + "&userid=" + this._userid + "&time=" + (long)time.TotalSeconds);
